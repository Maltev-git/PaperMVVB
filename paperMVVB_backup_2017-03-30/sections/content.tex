...
% 
% \begin{itemize}
% % 	\item We determine the fit function type for the parameters' frequency distributions with OMNI data.
% %	\item We determine the fit function type for the parameters' solar cycle dependency with OMNI and SSN data.
% % 	\item We determine the fit function type for the parameters' distance dependency with Helios data.
% 	\item Time dependent (yearly) distribution magnitudes versus solar activity. for OMNI
% % 	\item The function's behavior in direction of Sun down to \SI{0.0459}{\au} is evaluated.
% % 	\item We fit exponential (double) lognormal functions to (distance-binned) Helios data (Helios solar wind model).
% % 	\item We combine the obtained distance dependencies with the obtained solar activity relations (dynamical solar wind model).
% 	\item resulting dynamical model dependencies (distance, SSN)
% 	\item use extrapolated SSN (SILSO) of SPP mission for solar wind extrapolation (SPP solar wind estimation model).
% \end{itemize}
% 
...
\begin{table*}
	...
	\begin{tabular}{l@{} c
		...
		\multicolumn{2}{l}{}	&\multicolumn{1}{c}{$x_\text{med}$\tablefootmark{a}}	&\multicolumn{1}{c}{$x_\text{avg}$\tablefootmark{a}}	&\multicolumn{1}{c}{$c$}	&\multicolumn{1}{c}{[\%]}\\
		\hline
		\multicolumn{2}{l}{Magnetic field}	&5.661(16)	&6.164(18)	&--	&6.83\\
		\multicolumn{2}{l}{Velocity}	&4.085(19)	&4.183(20)	&--	&18.69\\
		\multicolumn{2}{l}{Density}	&5.276(24)	&6.484(34)	&--	&6.48\\
		\multicolumn{2}{l}{Temperature}	&7.470(17)	&11.301(32)	&--	&5.78\\
		\hline
		\multirow{2}{*}{Velocity}	&\multicolumn{1}{c}{$W_1$}	&4.89(14)	&5.00(14)	&\multirow{2}{*}{0.504(62)}	&\multicolumn{1}{c}{--}\\
			&\multicolumn{1}{c}{$W_2$}	&5.9089(53)	&3.72(20)	&	&\multicolumn{1}{c}{--}\\
		\cline{2-6}
			&\multicolumn{1}{c}{$W_\text{II}$}	&4.12(14)\tablefootmark{b}	&4.38(14)\tablefootmark{b}	&\multicolumn{1}{c}{--}	&4.20\\
		\hline
	\end{tabular}
	\tablefoot{
...
\end{figure*}
% From the normed stdfit values (Table~\ref{tab:single_lognormal_fit_parameters}) we see that the velocity fit is not that good... \\
% While by visual inspection it is obvious, but also fit stdfit is an indication for a bad fit...\\
% What we see at visual inspection is the area difference between the fit and the data. This area is for the velocity almost three times larger than the other's.\\
...
In the following sections we keep using the double lognormal ansatz for the velocity frequency fits...\\
...
\begin{figure}
	\resizebox{\hsize}{!}{\includegraphics{figures/OMNI_yearly_ssn_correlation_b_plot.pdf}}
...
To shift the distributions with SSN, we fit a linear SSN dependency to the median
\begin{align}
...
\begin{table*}
	\caption{The resulting fit coefficients from the data fitting with SSN. For the velocity the fit parameters from the double lognormal fit and their balancing function are given. The values in brackets are the estimated standard deviation of each fit parameter.}
	\label{tab:ssn_fit_parameters}
	\centering
%	\sisetup{table-figures-integer=1, table-figures-decimal=4, table-figures-exponent=1}
	\begin{tabular}{l@{} c@{}
...
In comparison with the data the fit models roughly match with the general trends (Fig.~\ref{fig:OMNI_yearly_BVNTSSN_fit_b_plot}).\\
\begin{figure*}
	\includegraphics[width=18cm]{figures/OMNI_yearly_BVNTSSN_fit_b_plot.pdf}
	\caption{Solar wind parameter frequencies, lognormal fit models and the corresponding yearly SSN (grey) over the OMNI time period 1963--2016. The for the model shifted SSN is indicated by a black line.}
	\label{fig:OMNI_yearly_BVNTSSN_fit_b_plot}
\end{figure*}
...
\begin{figure}
	\resizebox{\hsize}{!}{\includegraphics{figures/Vdbl_SSN_ratio_b_plot.pdf}}
	\caption{Ratio of slow to fast solar wind depending on the by 3~years lagged SSN. The relation results from the velocity's double lognormal fit.}
	\label{fig:Vdbl_SSN_ratio_b_plot}
\end{figure}

All fit coefficients are given in Table~\ref{tab:ssn_fit_parameters}).\\
...
\begin{figure}
	\resizebox{\hsize}{!}{\includegraphics{figures/OMNI_yearly_VSSN_dblfit_plot.pdf}}
...
To fit the bulk solar wind distributions' distance dependency we use the frequency fitting method from Sect.~\ref{sec:frequency_distribution} on distance-binned Helios data. This results in solar wind models comprising of exponentially with distance shifted lognormal functions.
...
\begin{table*}
	...
	S[table-format = +1.4(2)]}
		\hline\hline
		\multicolumn{2}{l}{Parameter}	&$c$	&\multicolumn{1}{c}{$a_\text{med}$\tablefootmark{a}}	&\multicolumn{1}{c}{$a_\text{avg}$\tablefootmark{a}}	&\multicolumn{1}{c}{$b$}\\
		\hline
		\multicolumn{2}{l}{Magnetic field}	&\multicolumn{1}{c}{--}	&5.358(25)	&5.705(28)	&-1.662(11)\\
		\multicolumn{2}{l}{Density}	&\multicolumn{1}{c}{--}	&5.424(33)	&6.845(47)	&-2.114(20)\\
		\multicolumn{2}{l}{Temperature}	&\multicolumn{1}{c}{--}	&6.357(64)	&10.72(14)	&-1.100(20)\\
		\hline
		\multirow{1}{*}{Velocity}	&$W''_1$	&\multirow{2}{*}{0.557(45)}	&3.707(13)	&3.748(16)	&\multirow{2}{*}{0.0990(51)}\\
			&$W''_2$	&	&5.26(13)	&5.42(11)	&\\
		\cline{2-6}
		\multicolumn{2}{r}{$W''_\text{II}$}	&\multicolumn{1}{c}{--}	&4.35(13)\tablefootmark{b}	&4.77(11)\tablefootmark{b}	&\multicolumn{1}{c}{--}\\
		\hline
	\end{tabular}
	\tablefoot{
		\tablefoottext{a}{Values in their respective units \si{nT}, \SI{e2}{\km\per\s}, \si{\per\cm\cubed} and \SI{e4}{\K}.}
...
With $c = 0.557$ the velocity balancing parameter is of an expected value similar to that obtained from the Helios time period (vgl.Fig.~\ref{fig:Vdbl_SSN_ratio_b_plot}). derive value...\\
...
Finally we combine the obtained solar activity and distance dependencies for shifting the frequency distributions. The result will be an empirical solar wind model for the inner heliosphere.\\
...
SPP orbit description, Venus flybys, ecliptic, \citep{Fox2015}, see Fig.~\ref{fig:SPP_orbit_predicted_SSN_overview_b_plot}\\
choosing SSN prediction source as input (SIDC + SWPC + previous cycle), (see Fig.~\ref{fig:SPP_orbit_predicted_SSN_overview_b_plot})\\
\begin{figure}
	\resizebox{\hsize}{!}{\includegraphics{figures/SPP_orbit_predicted_SSN_overview_b_plot.pdf}}
...
plot of SPP's estimated solar wind environment from predicted SSN (Fig.~\ref{fig:SPP_orbit_sw_forecast_BVNT_plot})\\
\begin{figure}
	\resizebox{\hsize}{!}{\includegraphics{figures/SPP_orbit_sw_forecast_BVNT_plot.png}}
	\caption{Estimated B-field median and frequency distribution during the mission time period 2024--2025.}
	\label{fig:SPP_orbit_sw_forecast_BVNT_plot}
\end{figure}
zoom into first perihelion (new figure...)\\

% The extrapolation of the general solar wind model of the SPP orbit range is plotted in Fig.~\ref{fig:sw_model_extrapolation_4_paper_plot}.\\
% \begin{figure*}
% 	{\includegraphics[width=17cm]{figures/sw_model_extrapolation_4_paper_plot.pdf}}
% 	\caption{Extrapolated solar wind parameters in the SPP observation range (\SIrange{0.0459}{0.25}{\au}). The median, mean and \SI{95.45}{\percent} range are extrapolated from the general solar wind model. use color scaled plot?}
% 	\label{fig:sw_model_extrapolation_4_paper_plot}
% \end{figure*}
...
The OMNI~2 data is time-shifted to the nose of the Earth's bow shock. This leads to yearly solar distance variations of \SI{>2}{\percent} (cite?) as the Earth orbits the Sun. Furthermore, its orbit within the ecliptic leads to a yearly variation of \SI{+-7.2}{\degree} in heliospheric latitude.\\
