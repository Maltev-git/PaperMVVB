...
Comparison with existing near-Sun models (Fig.~\ref{fig:sw_extrapolation_ssn_plot}); with half/twice SSN)\\
calculate or set min, avg and max SSN (e.g. 0, 100, 200)\\
\begin{figure*}
	\includegraphics[width=18cm]{figures/sw_extrapolation_ssn_plot.pdf}
	\caption{Solar wind model extrapolations to the PSP orbit region. The from Helios and OMNI measurements obtained models are extrapolated to the PSP region---for the case of solar minimum (maximum) with a SSN of 0 (200). For comparison other near-Sun models are also shown... to 1-column...?}
	\label{fig:sw_extrapolation_ssn_plot}
\end{figure*}
