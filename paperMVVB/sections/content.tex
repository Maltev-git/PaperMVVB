...
We extrapolate the derived empirical solar wind model (\ref{eq:general_sw_model}) to PSP’s orbital distance range and compare the results with those from the existing models shown in Fig.~\ref{fig:sw_extrapolation_ssn_d_plot}.
\begin{figure*}
	\includegraphics[width=18cm]{figures/sw_extrapolation_ssn_d_plot.pdf}
	...
	\label{fig:sw_extrapolation_ssn_d_plot}
\end{figure*}
...
\section{Summary}
\label{sec:summary}
For the near-Earth solar wind OMNI data and the Helios~1 and 2 data, obtained over the distance range \SIrange{0.29}{0.98}{\au}, we derived lognormal representations of the frequency distributions’ shapes of the four key solar wind parameters magnetic field strength, proton velocity, proton density and proton temperature. The dependencies of these frequency distributions on the solar activity cycle and on radial distance to the Sun have been modeled with analytical relations and extrapolated to the Parker Solar Probe orbit, taking into account predictions of the sunspot number. This empirical CGAUSS solar wind model for PSP, representing the solar wind’s solar activity and distance behavior, yields estimations for the solar wind median values at PSP’s first perihelion in 2018 at a solar distance of \SI{0.16}{\au} and at PSP’s closest perihelion in 2024 at \SI{0.046}{\au}. The model yields the following main results for the bulk solar wind parameters:
\begin{itemize}
	\item The empirical dependency of the magnetic field strength median value on solar activity and radial distance is
	\begin{align}
		B_\text{med}(ssn,r) = (\SI{0.0131}{\nT} \cdot ssn + \SI{4.29}{\nT}) \cdot r^{-1.66}	\,.
	\end{align}
	This approximation appears to be valid for PSP's full distance range, however near its closest perihelion the actual median values might lie within \SI{20}{\%} higher. The estimated median values for PSP's first and closest perihelion are \SI{87}{\nT} and \SI{943}{\nT}.
	\item The radial dependency of the median velocity value for slow solar wind is
	\begin{align}
		v_\text{slow}(r) &= \SI{363}{\km\per\s} \cdot r^{0.099}
		\intertext{and that for fast solar wind is}
		v_\text{fast}(r) &= \SI{483}{\km\per\s} \cdot r^{0.099}	\,.
	\end{align}
	These relations are valid above about \SI{20}{\Rs}, below this solar distance they overestimate the actual velocities obtained from remote measurements. Their combined frequency distribution (\ref{eq:double_lognormal_fit_function}) is balanced by $c(ssn) = -0.00180 \cdot ssn + 0.64$, that is, at solar minimum ($ssn = 0$) the slow wind has a share of \SI{64}{\%}, dropping to only \SI{28}{\%} during maximum ($ssn = 200$). The calculated median values for PSP's first and closest perihelion are \SI{340}{\km\per\s} and \SI{290}{\km\per\s}.
	\item The density relation is found to be
	\begin{align}
		n_\text{med}(ssn,r) = \left(\SI{0.0038}{\per\cm\cubed} \cdot ssn + \SI{4.50}{\per\cm\cubed}\right) \cdot r^{-2.11}	\,.
	\end{align}
	It seems valid throughout the full PSP orbital distance range, even down to about \SI{8}{\Rs}. The estimated median values for PSP's first and closest perihelion are \SI{4015}{\per\cm\cubed} and \SI{9733}{\per\cm\cubed}.
	\item The derived correlation function for the temperature is
	\begin{align}
		T_\text{med}(ssn,r) = (\SI{197}{\K} \cdot ssn + \SI{57300}{\K}) \cdot r^{-1.10}	\,.
	\end{align}
	The evaluation of the validity of this relation shows, that the temperature values near PSP's perihelion are too high in comparison to coronal measurements. The estimated median values for PSP's first and closest perihelion are \SI{503000}{\K} and \SI{1930000}{\K}.
\end{itemize}
...
The knowledge of the expected solar wind environment helps to optimize the WISPR and PSP preplanning of the science operations. This also applies for the Heliospheric Imager (SoloHI) onboard the Solar Orbiter spacecraft \citep{Howard2013}.
